...
|       P0 | Responsive fit       | 1024, 1280, and 1440 layouts must avoid page overflow                       | Local table scrolling, bounded panels, wrapping status/actions, and narrow full-screen approval sheet added                   | Matched in rc.6 production captures |
...
## rc.6 production verification

The exact rc.6 application commit `2d1de951f4f0122bb252187c74ddd557011069aa` was captured from `https://domhamster.netlify.app` in Chrome `151.0.7922.175`. Native-size 1440×960 evidence covers READY, the human-created locked conflict, exact-version approval, and COMMITTED audit states. Responsive captures at 1024×720, 1280×720, 1440×900, and the 1440×960 reference viewport showed no page-level horizontal overflow, clipped control, unreadable collapse, or contact value.

Originals and SHA-256 inventory: `W:\domhamster-release-evidence\media\rc6\screenshots\inventory.json`. The native Chrome report records the visual review and logic coverage at `W:\domhamster-release-evidence\chrome\rc6\REPORT.md`.
