...
const EXPECTED_COUNTS = Object.freeze({ READY: 5, DRAFT_INVALID: 7, DRAFT_VALID: 8, AWAITING_APPROVAL: 3, APPROVED: 4, COMMITTED: 3 });
...
for (const contract of contracts) {
  ...
  assert.ok(contract.title.length >= 8 && contract.title.length <= 80, `TITLE_LENGTH:${contract.name}`);
  assert.ok(contract.description.length >= 60 && contract.description.length <= 240, `DESCRIPTION_LENGTH:${contract.name}`);
...
for (const [state, expectedCount] of Object.entries(EXPECTED_COUNTS)) {
  ...
  for (const name of stateNames) assert.ok(names.includes(name), `LIFECYCLE_UNKNOWN_TOOL:${state}:${name}`);
}
const humanOnlyFragments = ['lock', 'unlock', 'approve', 'reject', 'cancel', 'discard', 'reset'];
for (const name of names) assert.equal(humanOnlyFragments.some((fragment) => name === fragment || name.startsWith(`${fragment}_`)), false, `HUMAN_ONLY_TOOL_EXPOSED:${name}`);
console.log(`DOMHAMSTER_TOOL_METADATA_PASS tools=${contracts.length} states=${Object.keys(lifecycle).length}`);
