...
walk(dist);
let rawBytes = 0; let gzipCodeBytes = 0; const inventory = [];
for (const path of files) { const bytes = readFileSync(path); rawBytes += bytes.length; const extension = extname(path); const gzipBytes = ['.js', '.css'].includes(extension) ? gzipSync(bytes).length : 0; gzipCodeBytes += gzipBytes; inventory.push({ path: relative(root, path), rawBytes: bytes.length, gzipBytes }); }
