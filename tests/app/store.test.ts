...
describe('serialized application store', () => {
  it(
    'reduces concurrent commands in call order and commits each state after persistence',
    async () => {
  ...
  it(
    'keeps the prior visible state when persistence fails and keeps the queue usable',
    async () => {
      let failWrites = true;
      let notifications = 0;
      const initial = readyState();
