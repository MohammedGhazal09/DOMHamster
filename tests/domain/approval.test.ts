...
describe('canonical human-approved journey', () => {
  it('preserves R-105 while the agent repairs R-106 before approval and commit', () => {
    const deps = dependencies();
    ...
    expect(state.draft!.assignments.find(({ requestId: id }) => id === requestId('R-105'))).toEqual(
      lockedR105,
    );
